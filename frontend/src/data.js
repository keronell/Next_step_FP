...
// Offline explainability: surface the answers that pushed this career highest as
// short "why this match" reasons, mirroring the backend's `reasons` field. Built
// from the same weights + bonuses the score uses, so reasons track the ranking.
function buildReasons(career, answers) {
  const weights = WEIGHTS[career.id] ?? {}
  return QUESTIONS
    .map((q) => {
      const val = answers[q.id]
      if (val === null || val === undefined) return null
      let contrib = val * (weights[q.id] ?? 0)
      BONUSES.forEach((b) => {
        if (b.careerId === career.id && b.qId === q.id && val === b.answerValue) contrib += b.bonus * 3
      })
      if (contrib <= 0) return null
      const option = q.options.find((o) => o.value === val)
      return option ? { contrib, label: option.label } : null
    })
    .filter(Boolean)
...
export function computeResults(answers) {
  ...
  return withPercent.map((c) => ({
    ...
    reasons: buildReasons(c, answers),
    // The client matcher can't infer which key skills you already have, so treat
    // them all as focus areas — this drives the roadmap's orange skill-gap nodes.
    missing_skills: c.keySkills ?? [],
    matched_skills: [],
  }))
}

// Curated market-demand overlay for the offline/standalone build. Mirrors the
// backend DEV-59 job-ad mining: tags roadmap nodes matching an in-demand skill
// with a `demand` payload so the "In demand · N%" / "Advantage" frames render
// with no backend. Keyed by career id (= roadmapKey). Percentages are
// illustrative, based on each career's real in-demand key skills.
export const MARKET_DEMAND = {
  'frontend': [
    { skill: 'React', classification: 'required', pct: 84 },
    { skill: 'TypeScript', classification: 'required', pct: 76 },
    { skill: 'CSS', classification: 'required', pct: 71 },
    { skill: 'Accessibility', classification: 'advantage', pct: 44 },
    { skill: 'Performance', classification: 'advantage', pct: 39 },
  ],
  'backend': [
    { skill: 'APIs', classification: 'required', pct: 82 },
    { skill: 'Databases', classification: 'required', pct: 79 },
    { skill: 'Node.js', classification: 'required', pct: 68 },
    { skill: 'System Design', classification: 'advantage', pct: 51 },
    { skill: 'Security', classification: 'advantage', pct: 47 },
  ],
  'data-science': [
    { skill: 'Python', classification: 'required', pct: 85 },
    { skill: 'SQL', classification: 'required', pct: 74 },
    { skill: 'Machine Learning', classification: 'required', pct: 69 },
    { skill: 'Statistics', classification: 'advantage', pct: 55 },
    { skill: 'Data Viz', classification: 'advantage', pct: 43 },
  ],
  'devops': [
    { skill: 'Kubernetes', classification: 'required', pct: 80 },
    { skill: 'CI/CD', classification: 'required', pct: 76 },
    { skill: 'Cloud', classification: 'required', pct: 72 },
    { skill: 'Terraform', classification: 'advantage', pct: 58 },
    { skill: 'Monitoring', classification: 'advantage', pct: 49 },
  ],
  'product-manager': [
    { skill: 'Strategy', classification: 'required', pct: 78 },
    { skill: 'User Research', classification: 'required', pct: 70 },
    { skill: 'Roadmapping', classification: 'required', pct: 64 },
    { skill: 'Metrics', classification: 'advantage', pct: 52 },
    { skill: 'Storytelling', classification: 'advantage', pct: 45 },
  ],
  'ux-designer': [
    { skill: 'Figma', classification: 'required', pct: 82 },
    { skill: 'User Research', classification: 'required', pct: 71 },
    { skill: 'Prototyping', classification: 'required', pct: 66 },
    { skill: 'Design Systems', classification: 'advantage', pct: 54 },
    { skill: 'Usability Testing', classification: 'advantage', pct: 48 },
  ],
  'fullstack': [
    { skill: 'JavaScript', classification: 'required', pct: 83 },
    { skill: 'React', classification: 'required', pct: 78 },
    { skill: 'Node.js', classification: 'required', pct: 70 },
    { skill: 'SQL', classification: 'required', pct: 66 },
    { skill: 'MongoDB', classification: 'advantage', pct: 44 },
  ],
  'mobile': [
    { skill: 'Swift', classification: 'required', pct: 68 },
    { skill: 'Kotlin', classification: 'required', pct: 66 },
    { skill: 'Flutter', classification: 'required', pct: 62 },
    { skill: 'iOS', classification: 'advantage', pct: 58 },
    { skill: 'Android', classification: 'advantage', pct: 57 },
  ],
  'data-analyst': [
    { skill: 'SQL', classification: 'required', pct: 86 },
    { skill: 'Excel', classification: 'required', pct: 72 },
    { skill: 'Python', classification: 'required', pct: 68 },
    { skill: 'Tableau', classification: 'advantage', pct: 56 },
    { skill: 'Power BI', classification: 'advantage', pct: 51 },
  ],
  'machine-learning': [
    { skill: 'Python', classification: 'required', pct: 87 },
    { skill: 'PyTorch', classification: 'required', pct: 73 },
    { skill: 'TensorFlow', classification: 'required', pct: 67 },
    { skill: 'SQL', classification: 'advantage', pct: 58 },
    { skill: 'MLOps', classification: 'advantage', pct: 49 },
  ],
  'ai-engineer': [
    { skill: 'Python', classification: 'required', pct: 85 },
    { skill: 'LLM', classification: 'required', pct: 79 },
    { skill: 'RAG', classification: 'required', pct: 68 },
    { skill: 'PyTorch', classification: 'advantage', pct: 57 },
    { skill: 'NLP', classification: 'advantage', pct: 52 },
  ],
  'cyber-security': [
    { skill: 'Penetration Testing', classification: 'required', pct: 74 },
    { skill: 'Python', classification: 'required', pct: 66 },
    { skill: 'Nmap', classification: 'required', pct: 58 },
    { skill: 'Wireshark', classification: 'advantage', pct: 51 },
    { skill: 'Kali Linux', classification: 'advantage', pct: 49 },
  ],
  'qa-engineer': [
    { skill: 'Test Automation', classification: 'required', pct: 76 },
    { skill: 'Selenium', classification: 'required', pct: 71 },
    { skill: 'Python', classification: 'required', pct: 63 },
    { skill: 'JUnit', classification: 'advantage', pct: 48 },
    { skill: 'JIRA', classification: 'advantage', pct: 44 },
  ],
  'game-dev': [
    { skill: 'Unity', classification: 'required', pct: 72 },
    { skill: 'C#', classification: 'required', pct: 68 },
    { skill: 'Unreal Engine', classification: 'required', pct: 58 },
    { skill: 'Game Design', classification: 'advantage', pct: 54 },
    { skill: 'OpenGL', classification: 'advantage', pct: 41 },
  ],
  'technical-writer': [
    { skill: 'Documentation', classification: 'required', pct: 79 },
    { skill: 'Markdown', classification: 'required', pct: 68 },
    { skill: 'APIs', classification: 'required', pct: 60 },
    { skill: 'Git', classification: 'advantage', pct: 52 },
    { skill: 'SEO', classification: 'advantage', pct: 43 },
  ],
  'software-architect': [
    { skill: 'System Design', classification: 'required', pct: 82 },
    { skill: 'Microservices', classification: 'required', pct: 74 },
    { skill: 'AWS', classification: 'required', pct: 70 },
    { skill: 'Kubernetes', classification: 'required', pct: 63 },
    { skill: 'Docker', classification: 'advantage', pct: 58 },
  ],
}
