...
# backend/app/services/requirements_service.py -> repo root is four parents up.
_REPO_ROOT = Path(__file__).resolve().parents[3]
_SKILL_ALIASES_PATH = _REPO_ROOT / "data" / "config" / "skill_aliases.json"
...
class RequirementsService:
    """Mines the RAG store for a career field's in-demand skills. Never raises."""

    def __init__(self, rag, settings: Settings):
        self._rag = rag
    ...
    def get_requirements(self, career_id: str) -> dict:
        try:
            ...
            if not field:
                return _empty()
            s = self._settings
            counts, n_ads = self._rag.field_skills(field, s.requirements_sample_size)
