...
def test_login_when_disabled(client):
    r = client.post(
...
def test_me_invalid_token_raises_401(client, monkeypatch):
    monkeypatch.setattr(
        auth_service,
        "get_user_from_token",
...
def test_logout_when_disabled(client, monkeypatch):
...
def test_register_success(client, auth_client):
    r = client.post(
        "/api/auth/register",
...
def test_logout_success(client, monkeypatch, auth_client):
    monkeypatch.setattr(
        auth_service,
        "get_user_from_token",
        lambda jwt: auth_service.UserResponse(
...
def test_register_missing_username_rejected(client):
    r = client.post(
...
def test_register_duplicate_username_rejected(client, monkeypatch):
