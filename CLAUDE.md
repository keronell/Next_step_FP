...
cd backend && venv/bin/python -m pytest app/tests -q          # 46 tests, mock the vector store + Supabase (no ChromaDB/DB needed)
```
...
**Auth context:** `frontend/src/contexts/AuthContext.jsx` provides `AuthProvider` (mounted in `main.jsx` wrapping `<App>`) and the `useAuth()` hook. `useAuth()` returns `{ user, authLoading, signIn, signUp, signOut }`. `user` is `{ user_id, email }` or `null`; `authLoading` is `true` during the initial `GET /api/auth/me` rehydration on mount (prevents a "Sign In" flash for returning users). Tokens are stored in localStorage as `nextstep_access_token` and `nextstep_refresh_token`. `api.js` exposes a `_request` helper that auto-attaches `Authorization: Bearer <token>` to every fetch — no call site needs to handle this manually. New `api.js` exports: `signUp`, `signIn`, `signOut`, `getMe`, `claimSessions`, `fetchMySubmissions`.
