...
Implications:
- To change the careers/weights, update **both** `frontend/src/data.js` (the offline fallback) and `backend/app/data/careers.json` (the live matcher), or they drift. Same for roadmaps: `frontend/src/data.js` `ROADMAPS` **and** `backend/app/data/roadmaps.json`. Same for questions: `frontend/src/data.js` `QUESTIONS`/`WEIGHTS`/`BONUSES` **and** `backend/app/data/questions.json`+`careers.json` — the SPA fetches `GET /api/questions` on load (`Questionnaire.jsx`) and falls back to the bundled `QUESTIONS`. `showIf` is declarative (`{q, in}`, serialized as `show_if` in the JSON); new questions must be **appended** (never inserted) so a fetch resolving mid-quiz keeps indices stable. q11+ are pure discriminators: zero weights, signal via per-option bonuses only. Adding/removing questions changes the learned-matcher feature layout — bump `FEATURE_VERSION` in `feature_builder.py` (`QUESTION_IDS` derives from `questions.json`); the old artifact is then refused and matching falls back to the formula until retrained. `backend/app/tests/test_question_bank.py` enforces the bank's DoD: no two questions share a matching signal, every career ranks #1 for its archetype.
...
cd backend && venv/bin/python -m pytest app/tests -q          # 83 tests, mock the vector store + Supabase (no ChromaDB/DB needed)
```
...
- **Roadmap layout & status colors (DEV-46/DEV-47):** `Roadmap.jsx` renders sections as a left-to-right **pipeline** — plain HTML/flex columns (one per section, header pill + nodes stacked below) separated by a tall gold vertical divider, horizontally scrollable, no fixed canvas size (scales to any section/node count, not hardcoded to 6 careers). There is no SVG canvas or connector-line drawing anymore; node reveal is a staggered `framer-motion` fade-in keyed on `selectedCareer`. Node color now encodes **status**, not difficulty level: green = mastered (node label matches a `matched_skills` entry, or the node is in `completedNodes`), red = skill gap (matches `missing_skills`), orange = recommended next (everything else) — computed by `nodeStatus()`/`skillHits()` in `Roadmap.jsx`, which tokenizes and canonicalizes skill/label words (a small `TOKEN_ALIASES` map, e.g. `sql→database`, `api→rest`, `viz→visualization`) so e.g. `"SQL"` matches the `"Databases"` node. `App.jsx` passes both `missingSkills` and `matchedSkills` (from the selected result's `missing_skills`/`matched_skills`) into `Roadmap`; offline/fallback results carry neither, so nodes default to orange unless completed. Node *type* (required/good-to-know/optional) still renders as shape (solid/outline/dashed) independent of status color. A legend above the canvas explains the 3 status colors plus the separate gold ring + check badge (manual completion) — the two systems are deliberately kept visually distinct.
